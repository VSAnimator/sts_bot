...
def similarity_query(current_state, additional_filters):
    try:
        query = f"""
        select states.*
        from runs, states
...
def event_query(current_state):
    print("Event name:", current_state['game_state']['screen_state']['event_name'])
    return similarity_query(current_state, f'''event_name == "{current_state['game_state']['screen_state']['event_name']}"''')

def card_choice_query(current_state):
    ...
    return similarity_query(current_state, choices_condition)

def campfire_query(current_state):
...
def smithing_query(current_state):
    return similarity_query(current_state, "1 == 1")

def map_query(current_state):
    return similarity_query(current_state, "1 == 1")
...
test_states = {
    ...
    'campfire': {'available_commands': ['choose'], 'ready_for_command': True, 'game_state': {'choice_list': ['rest', 'smith', 'recall'], 'screen_type': 'REST', 'screen_state': {'has_rested': False, 'rest_options': ['rest', 'smith', 'recall']}, 'seed': 5765269433870820538, 'deck': ['AscendersBane_upg0', 'Strike_R_upg0', 'Strike_R_upg0', 'Strike_R_upg0', 'Defend_R_upg0', 'Defend_R_upg0', 'Defend_R_upg0', 'Defend_R_upg0', 'True Grit_upg0', 'Anger_upg0', 'Power Through_upg0', 'Iron Wave_upg0', 'Metallicize_upg0', 'Body Slam_upg0', 'Pummel_upg0', 'Shrug It Off_upg0', 'Feed_upg0', 'Sword Boomerang_upg0'], 'relics': ['Burning Blood', 'Bottled Flame', 'Red Skull', 'MawBank'], 'max_hp': 80, 'act_boss': 'Hexaghost', 'gold': 251, 'action_phase': 'WAITING_ON_USER', 'act': 1, 'screen_name': 'NONE', 'room_phase': 'INCOMPLETE', 'is_screen_up': False, 'potions': ['PowerPotion', 'Strength Potion', 'FearPotion'], 'current_hp': 8, 'floor': 15, 'ascension_level': 10, 'class': 'IRONCLAD', 'choice_info': {}}},
...
def test_query(query_name):
    ...
    return globals()[query_name + "_query"](test_states[query_name])
